...
public class WallRoad : MonoBehaviour
{
    ...
    [SerializeField]
    private Vector3 wallCubeSize = new Vector3(1, 1, 0.2f);
    ...
    private void CreateWall(){
        ...
        for(int height = 0; height < wallHeightSize; height++ ){
            for(int length = 0; length < wallLengthSize; length++){
                ...
                cubeObject.name = "CubeRoad " + height + "-" + length;
                cubeObject.AddComponent<CubeRoad>();
                cubeObject.GetComponent<CubeRoad>().UpdateCubeSize(wallCubeSize);
                cubeObject.transform.parent = this.transform;
                cubeObject.transform.position = nextPosition; 
                nextPosition.x = nextPosition.x + (cubeObject.GetComponent<CubeRoad>().CubeSize().x * 2);
                cubeHeight = (cubeObject.GetComponent<CubeRoad>().CubeSize().y * 2);
            }
            nextPosition.x = 0;
